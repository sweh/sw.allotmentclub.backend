        ...
        assignments = (
            self.db.query(
                ...
                to_string(Member.lastname).concat(', ').concat(
                    to_string(Member.firstname)).label('Verantwortlich'))
...
@view_config(route_name='assignments',
             renderer='json',
             permission='view')
class AssignmentListView(sw.allotmentclub.browser.base.TableView):
    ...
    start_year = 2015
    available_actions = [
        dict(url='assignment_add', btn_class='btn-success',
             icon='fa fa-plus', title='Neu'),
        dict(url='assignment_edit', btn_class='btn-success',
             icon='fa fa-pencil', title='Bearbeiten'),
        dict(url='assignment_delete', btn_class='btn-danger',
             icon='glyphicon glyphicon-trash', title='Löschen',
             callback='sw.allotmentclub.assignments_list_view.render()'),
        dict(url='assignment_list_attendees', btn_class='btn-success',
             icon='fa fa-list', title='Teilnehmer')]


@view_config(route_name='assignment_edit', renderer='json',
             permission='view')
...
class AssignmentAddView(AssignmentEditView):
    ...
    def __init__(self, context, request):
        ...
        log_with_user(user_data_log.info, self.request.user,
                      'Arbeitseinsatz %s hinzugefügt.', self.context.id)
...
@view_config(route_name='assignment_delete', renderer='json',
             permission='view')
    ...
    def select(self):
        return (
            self.db.query(
                ...
                to_string(Member.lastname).concat(', ').concat(
                    to_string(Member.firstname)).label('Teilnehmer'),
...
@view_config(route_name='assignment_list_attendees', renderer='json',
             permission='view')
class AssignmentListAttendeesView(sw.allotmentclub.browser.base.TableView):
    ...
    available_actions = [
        dict(url='assignment_attendees_add', btn_class='btn-success',
             icon='fa fa-plus', title='Neu'),
        dict(url='assignment_attendees_edit', btn_class='btn-success',
             icon='fa fa-pencil', title='Bearbeiten'),
        dict(url='assignment_attendees_delete', btn_class='btn-danger',
             icon='glyphicon glyphicon-trash', title='Löschen',
             callback='sw.allotmentclub.assignment_attendees_view.render()')]


@view_config(route_name='assignment_attendees_edit', renderer='json',
             permission='view')
...
@view_config(route_name='assignment_attendees_add', renderer='json',
             permission='view')
...
@view_config(route_name='assignment_attendees_delete', renderer='json',
             permission='view')
    ...
    def log(self):
        if self.deleted is not None:
            ...
            log_with_user(user_data_log.info,
                          ...
                          deleted.firstname, deleted.lastname, assignment)
...
@view_config(route_name='member_assignments', renderer='json',
             permission='view')
class MemberAssignmentView(sw.allotmentclub.browser.base.TableView):
    ...
    available_actions = [
        dict(url='member_assignments_detail', btn_class='btn-success',
             icon='fa fa-list', title='Details'),
        dict(url='member_assignments_bill', btn_class='btn-danger',
             icon='fa fa-bank', title='Abrechnen')]
...
@view_config(route_name='member_assignments_detail', renderer='json',
             permission='view')
...
@view_config(route_name='member_assignments_bill', renderer='json',
             permission='view')
