...
from .. import Allotment, BookingKind, Member, User
...
def get_view_for_route(route_name, request):
    ...
    view = request.registry.adapters.lookup(
        ...
        pyramid.interfaces.ISecuredView,
        default=None,
    )
    if view is None:
        raise TypeError(
            "The requested route `{}` is not configured.".format(route_name)
        )
    return view
...
def string_agg(col, delimiter="/"):
    return sqlalchemy.func.string_agg(
        to_string(col), to_string(delimiter), order_by=col.asc()
    )
...
class View(object):
    ...
    def form_submit(self):
        return self.request.params and list(self.request.params.keys()) != [
            "for_year"
        ]

    @property
    def active_members(self):
        return Member.query().join(Allotment, Allotment.member_id == Member.id)
        ...
        for item in form_attributes:
            ...
            if "values" in item:
                ...
                for value in item["values"]:
                    value = value.copy()
                    if (
                        getattr(self.context, item["name"], None)
                        == value["id"]
                    ):
...
class EditView(AddEditBase):
    def update(self):
        if self.form_submit():
            ...
            result = {"status": "success"}
            result.update(
                {"data": {"form": form_attributes, "title": self.title}}
            )
        self.result = result
...
class EditJSFormView(AddEditBase):
        ...
        return [
            {
                ...
                "title": "{}, {} ({})".format(
                    member.lastname,
                    member.firstname,
                    "/".join(str(a.number) for a in member.allotments)
                    or "n/a",
                ),
            }
    ...
    def users(self):
        return (
            ...
            .filter(
                sqlalchemy.or_(User.is_locked.is_(None), User.is_locked == "")
            )
        )
        ...
        return [
            {
                "token": user.id,
                "title": "{}, {} ({})".format(
                    user.nachname, user.vorname, user.username
                ),
            }
    ...
    def get_route(self, item, route):
        return "/api/" + route_url(route, self.request).replace(
            "{id}", str(item.id)
        )
    ...
    def save(self, key, value):
        try:
            ...
            type_ = None
        if isinstance(type_, sqlalchemy.DateTime) or isinstance(
...
class Query(object):
    ...
    def _assert_columns(self, query, columns, kind):
        ...
        for col in columns:
            assert (
                col in query_string
            ), "Column {!r} is in self.{} but not in query.".format(col, kind)
...
class XLSXExporterView(View):
    ...
    def __call__(self):
        ...
        response.content_disposition = "attachment; filename={}.xlsx".format(
            self.filename
        )
...
class SEPAExporterView(CSVExporterView):
    ...
    def data_wire_bank(self):
        ...
        for value in self.values:
            ...
            if member.direct_debit_account_holder:
                ...
                name = "{} {}, {}".format(
                    member.lastname, member.title, member.firstname
                )
            else:
    ...
    def data_direct_debit(self):
        ...
        for value in self.values:
            ...
            id_ = "BNGLW{}".format(
                "-".join(str(m.number) for m in member.allotments)
            )
    ...
    def __call__(self):
        ...
        response.content_disposition = "attachment; filename={}.xml".format(
            self.filename
        )
